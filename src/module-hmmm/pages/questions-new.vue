  ...
  <div class="container">
    <el-card>
      ...
      <el-row style="padding-left: 120px">
        <el-button
          :type="this.formData.id ? 'success' : 'primary'"
          @click="btnOK"
          >{{ this.formData.id ? "确认修改" : "确认提交" }}</el-button
        >
      </el-row>
    </el-card>
  </div>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        city: [{ required: true, message: "本项不能为空", trigger: "blur" }],
        direction: [
          { required: true, message: "本项不能为空", trigger: "blur" },
        ],
        questionType: [
        ...
        answer: [{ required: true, message: "本项不能为空", trigger: "blur" }],
      },
    };
  },
  computed: {
