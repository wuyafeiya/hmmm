  ...
  <div class="container">
    ...
    <el-dialog :visible.sync="showDialog" :title="dirtitle" width="400px" @close="btnCancel">
      ...
      <el-form-item prop="directoryName" label="目录名称">
        <el-input type="text" v-model="formData.directoryName" placeholder="请输入目录名称" />
      </el-form-item>
      </el-form>
...
<script>
import { list, add, remove, update } from '../../api/hmmm/directorys.js'
...
export default {
  ...
  methods: {
    // 总列表
    async lists () {
      // const { data } = await directorylist(this.pages)
    ...
    // 筛选 搜索 和 清除
    async search () {
      // this.searchList = this.list.filter(item => (item.directoryName || item.state) == (this.searchData.basenameStatus || this.searchData.baseApiStatus))
      if (this.list.length > 10) {
        this.searchList = this.list.filter(item => {
          if (this.searchData.basenameStatus > '' && this.searchData.baseApiStatus == '') {
            return item.directoryName == this.searchData.basenameStatus
          } else if (this.searchData.basenameStatus == '' && this.searchData.baseApiStatus > '') {
            return item.state == this.searchData.baseApiStatus
          } else if (this.searchData.basenameStatus > '' && this.searchData.baseApiStatus > '') {
            return item.directoryName == this.searchData.basenameStatus && item.state == this.searchData.baseApiStatus
          }
        })
